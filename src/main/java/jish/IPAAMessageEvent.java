...
public class IPAAMessageEvent {
    ...
    public void userLeft(UserLeaveEvent event) {
        ...
        IChannel main = event.getClient().getChannelByID(JishBit.MAIN_CH_ID);
        if (server.equals(JishBit.MAIN_CH_ID)) {
    ...
    public void userJoined(UserJoinEvent event) {
        ...
        IGuild ipa = JishBit.getClient().getGuildByID(JishBit.MAIN_CH_ID);
        Util.sendMessage(main, "Holy fuck look who's back it's " + joinGuy.toString());

        if (server.equals(JishBit.getClient().getGuildByID(JishBit.MAIN_CH_ID))) {
